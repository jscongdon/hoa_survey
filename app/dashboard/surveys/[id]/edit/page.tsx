              ...
              <div>
                ...
                <div className="flex items-start gap-2">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const ok = window.confirm(
                        "Changing the member list will affect which members receive this survey. Continue?"
                      );
                      if (!ok) return;
                      const name =
                        lists.find((l) => l.id === nextId)?.name || "";
                      setMemberListId(nextId);
                      setMemberListName(name);
                    }}
                  >
                    <option value="" disabled>
                      Select a member list
                    </option>
                    {lists.map((l) => (
                  ...
                  {submittedResponses > 0 && (
                    <span
                      className="text-xs mt-2 px-2 py-1 rounded bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300"
              ...
              <div>
                <label className="block text-sm mb-2 text-gray-900 dark:text-white">
                  ...
                  <span className="ml-2 text-xs text-gray-500 dark:text-gray-400">
                    ({submittedResponses} of {totalResponses} responses
                    submitted)
                  </span>
                </label>
                {submittedResponses > 0 ? (
                  <div className="space-y-3">
                    <div className="bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 rounded p-3 mb-3">
                      <p className="text-sm text-yellow-800 dark:text-yellow-200">
                        🔒 Questions cannot be edited after responses have been
                        submitted.
                      </p>
                    </div>
                    {questions.map((q, i) => (
                  ...
                  <div>
                    <div className="bg-blue-50 border border-blue-200 rounded p-3 mb-3">
                      <p className="text-sm text-blue-800">
                        ✏️ You can add, edit, or remove questions below. Changes
                        will be saved when you click "Update Survey".
                      </p>
                    </div>
                    <SurveyBuilder
