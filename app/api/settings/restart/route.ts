...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      try {
        const { stdout } = await execAsync('docker ps --format "{{.Names}}" | grep -E "hoa_survey|app"')
    ...
    return NextResponse.json({ 
      success: false,
      message: 'Unable to restart automatically. Please manually restart the application: docker compose restart' 
    }, { status: 200 })
