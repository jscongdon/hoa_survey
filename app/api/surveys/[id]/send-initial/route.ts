...
export async function POST(
  ...
  try {
    ...
    const survey = await prisma.survey.findUnique({ where: { id }, include: { memberList: true } });
    if (!survey) return NextResponse.json({ error: 'Not found' }, { status: 404 });

    if (survey.initialSentAt) {
      return NextResponse.json({ error: 'Initial notice already sent' }, { status: 409 });
    }

    // Load responses and member info
    const responses = await prisma.response.findMany({ where: { surveyId: id }, include: { member: true } });

    // Determine base URL
    const isDevelopment = process.env.NODE_ENV === 'development';
    const baseUrl = isDevelopment ? (process.env.DEVELOPMENT_URL || 'http://localhost:3000') : (process.env.PRODUCTION_URL || '');
    ...
    // Mark survey as sent
    await prisma.survey.update({ where: { id }, data: { initialSentAt: new Date() } });

    return NextResponse.json({ ok: true });
  } catch (error) {
    logError('[SEND_INITIAL]', error);
    return NextResponse.json({ error: 'Failed to send initial notices' }, { status: 500 });
  }
}
