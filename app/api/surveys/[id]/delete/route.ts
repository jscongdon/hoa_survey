...
export async function DELETE(
  ...
  try {
    ...
    const submittedCount = survey.responses.filter(
      ...
      try {
        ...
        if (!admin || admin.role !== "FULL") {
          return NextResponse.json(
            { error: "Insufficient permissions for forced delete" },
            { status: 403 }
          );
        }
        ...
        logError(
          "[DELETE_SURVEY] Error fetching admin role for force delete",
          e
        );
        return NextResponse.json(
          { error: "Insufficient permissions for forced delete" },
