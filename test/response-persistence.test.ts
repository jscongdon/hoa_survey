...
describe('response persistence transaction', () => {
  ...
  beforeAll(async () => {
    const ml = await prisma.memberList.create({ data: { name: 'test-list' } })
    memberListId = ml.id

    const member = await prisma.member.create({
      data: { lot: '1', name: 'Test Member', email: `test+${Date.now()}@example.com` },
    })
    memberId = member.id
    ...
    const resp = await prisma.response.create({
      data: { surveyId: surveyId, memberId: memberId, token: `t-${Date.now()}` },
    })
    responseId = resp.id
  })

  afterAll(async () => {
    if (responseId) await prisma.response.deleteMany({ where: { id: responseId } })
    ...
    if (memberListId) await prisma.memberList.deleteMany({ where: { id: memberListId } })
  ...
  it('inserts answers and updates response in a transaction', async () => {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.response.update({ where: { id: responseId! }, data: { submittedAt: new Date() } })
    })

    const answers = await prisma.answer.findMany({ where: { responseId } })
