...
import { userSchema, loginSchema, surveySchema, questionSchema, inviteSchema, responseSchema } from '../lib/validation/schemas'

// User schema tests
describe('userSchema', () => {
  it('validates a correct user', () => {
    expect(() => userSchema.parse({ email: 'a@b.com', password: 'password123' })).not.toThrow()
  })
  it('rejects invalid email', () => {
    expect(() => userSchema.parse({ email: 'bad', password: 'password123' })).toThrow()
  })
  it('rejects short password', () => {
    expect(() => userSchema.parse({ email: 'a@b.com', password: 'short' })).toThrow()
  })
})
...
describe('questionSchema', () => {
  ...
  it('rejects missing text', () => {
    expect(() => questionSchema.parse({ type: 'PARAGRAPH', order: 0 })).toThrow()
  })
})

describe('surveySchema', () => {
  it('validates a survey with questions', () => {
    expect(() => surveySchema.parse({
      title: 'Test',
      questions: [
...
describe('inviteSchema', () => {
  it('validates admin invite', () => {
    expect(() => inviteSchema.parse({ email: 'a@b.com', role: 'admin' })).not.toThrow()
  })
